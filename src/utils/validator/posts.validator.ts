...
const postType = ["public", "private", "friends"];
...
export const createPostValidator = [
  ...
  check("postType").custom((val) => {
    if (!postType.includes(val)) {
      throw new APIError(`${val} not exist in post type keywords!!`, 404);
    }
    return true;
  }),
  validatorMW,
  ...
  check("id")
    ...
    .custom(async (val, { req }) => {
      const isPostExist = await errorHandling(
        Post.exists({ _id: val, userId: req.user._id }).lean().exec()
      );
