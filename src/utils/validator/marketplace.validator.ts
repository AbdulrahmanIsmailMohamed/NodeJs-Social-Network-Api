...
const availability: Array<string> = [Availability[0], Availability[1]];
...
export const createItemForSaleValidator = [
  ...
  check("availability")
    ...
    .custom((val) => {
      ...
      throw new APIError(
        "Availability must contain a 'View As A Single' Or \n'Item view As Available' !!",
        400
      );
    }),
  check("category")
...
export const itemForSaleValidator = [
  check("id")
    ...
    .custom(async (val, { req }) => {
      const isItemForSaleExist = await errorHandling(
        Marketplace.exists({ _id: val, userId: req.user._id }).exec()
      );
