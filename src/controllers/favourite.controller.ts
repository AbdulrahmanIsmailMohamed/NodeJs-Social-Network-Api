...
import { FavouriteData } from "../interfaces/favourite.interface";
...
export class FavouriteController {
  ...
  addFavourite = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      if (req.user) {
        ...
        const addFavourite = await this.FavouriteService.addFavourite(
          favouriteData
        );

        if (!addFavourite) {
          return next(
            new APIError("Can't Add this post in your favourite", 400)
          );
        }
  ...
  favourites = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      if (req.user) {
        const userId: string = req.user._id as string;

        const favourites = await this.FavouriteService.favourites(userId);
        if (!favourites) {
          return next(
            new APIError("Can't Add this post in your favourite", 400)
          );
  ...
  deleteFavourite = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      if (req.user) {
        ...
        const deleteFavourite = await this.FavouriteService.deleteFavourite(
          favouriteData
        );
        if (!deleteFavourite) {
          return next(
