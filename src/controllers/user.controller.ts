...
import { UpdateLoggedUser } from "../interfaces/user.Interface";
...
class UserController {
  ...
  updateLoggedUser = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      if (req.user) {
        ...
        const user = await this.userService.updateLoggedUser(userBody);
  ...
  getUsers = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      const result = await this.userService.getUsers(features);
  ...
  getUser = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      const userId: string =
        req.params.id ||
        (req.user && (req.user._id as string)) ||
        "please login";
  ...
  inactiveLoggedUser = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      if (req.user) {
        const user = await this.userService.inActiveLoggedUser(
